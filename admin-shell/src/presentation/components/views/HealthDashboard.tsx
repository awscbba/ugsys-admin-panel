...
import { $healthStatuses, loadHealthStatuses } from '../../../stores/healthStore';
...
const STATUS_COLORS: Record<HealthState, { bg: string; border: string; text: string; dot: string }> = {
  healthy:   { bg: '#f0fdf4', border: '#86efac', text: '#15803d', dot: '#22c55e' },
  degraded:  { bg: '#fefce8', border: '#fde047', text: '#a16207', dot: '#eab308' },
  unhealthy: { bg: '#fef2f2', border: '#fca5a5', text: '#b91c1c', dot: '#ef4444' },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <dt style={{ color: '#6b7280', marginBottom: '2px' }}>Response time</dt>
          <dd style={{ margin: 0, color: '#374151', fontWeight: 500 }}>
        ...
        {entry.statusCode !== undefined && (
          <div>
            <dt style={{ color: '#6b7280', marginBottom: '2px' }}>Status code</dt>
            <dd style={{ margin: 0, color: '#374151', fontWeight: 500 }}>
              {entry.statusCode}
            </dd>
          </div>
...
export function HealthDashboard() {
  ...
  const fetchStatuses = async () => {
    ...
    logger.logComponentEvent({ event: 'fetch_start', component: 'HealthDashboard' });

    try {
      await loadHealthStatuses();
      logger.logComponentEvent({ event: 'fetch_success', component: 'HealthDashboard' });
    } catch (err) {
    ...
    return (
      ...
      >
        ...
        <p style={{ margin: '8px 0 0', fontSize: '14px' }}>
          You need the <strong>admin</strong> or <strong>super_admin</strong> role to view the health dashboard.
        </p>
      </div>
    );
  }
