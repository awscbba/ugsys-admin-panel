...
import { HttpUserProfileClient } from "../../../infrastructure/repositories/HttpUserProfileClient";
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              cursor:
                isSaving || selected.length === 0 ? "not-allowed" : "pointer",
...
export function UserManagement() {
  ...
  const upsClient = useRef(new HttpUserProfileClient());
  ...
  const handleRoleConfirm = async (roles: string[]) => {
    ...
    logger.logUserAction({
      action: "change_roles",
      context: { userId: roleModalUser.userId },
    });
  ...
  const handleToggleStatus = async (user: AdminUser) => {
    ...
    logger.logUserAction({
      ...
      context: { userId: user.userId, newStatus },
    });
  ...
  return (
    <div>
      ...
      {/* Row-level UPS fetch error banners */}
      {Object.entries(rowErrors).map(([userId, msg]) => (
        <div
          key={userId}
          role="alert"
          style={{
              ...
              {users.length === 0 ? (
                ...
                users.map((user) => (
                  <UserRow
                    ...
                    onEdit={handleEdit}
                    actionInProgress={actionInProgress ?? editLoadingUserId}
                  />
                ))
              )}
            </tbody>
